    ...
    if (server != nullptr) {
        ...
        while (enet_host_service(server, &event, 1000000) > 0) {
            ...
            case ENET_EVENT_TYPE_RECEIVE: {
                std::vector<unsigned char> receivedData(event.packet->data, event.packet->data + event.packet->dataLength);
...
void Server::sendData(const unsigned char* byteArray, size_t size) {
    if (clientPeer != nullptr) {
        ENetPacket* packet = enet_packet_create(byteArray, size, ENET_PACKET_FLAG_RELIABLE);
